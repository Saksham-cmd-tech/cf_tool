...
class CFExplore:
    ...
    def load(self):
        ...
        solved   = all_solved()   # set of normalized uppercase IDs
    ...
    def render(self, window: int = 30) -> FormattedText:
        ...
        for i in range(start, end):
            ...
            tokens.append(("class:rating.green" if p["solved"] else "", " ✓  " if p["solved"] else "    "))
    ...
    def run(self, initial_query: str | None = None, no_cache: bool = False):
        ...
        results_control = FormattedTextControl(text=lambda: self.render(), focusable=False)
        ...
        status = Window(
            content=FormattedTextControl(
                text=lambda: FormattedText([
                    ("", f"  ↑↓ navigate  Enter open  Ctrl-C quit  —  "
                         f"{self.index + 1}/{len(self.filtered)}  "),
                ])
            ),
            height=1,
        )
